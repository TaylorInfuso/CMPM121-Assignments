...
public class CameraFollow2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (gameObject.GetComponentInParent<PlayerMovement2>().flat == false)
        {
            ...
            this.transform.rotation = Quaternion.Slerp(this.transform.rotation, Quaternion.Euler(0, (player.GetComponent<PlayerMovement2>().rotationLeftAndRight) / Mathf.PI * 180, 0), Time.deltaTime * 10.0f);
        }
        else
        {
            angle = this.GetComponent<Transform>().localEulerAngles;
            ...
            this.GetComponent<Transform>().localEulerAngles = angle;
            //playCam.transform.rotation = Quaternion.Slerp(playCam.transform.rotation, Quaternion.Euler(90, (player.GetComponent<PlayerMovement2>().rotationLeftAndRight) / Mathf.PI * -180, 0), Time.deltaTime * 10.0f);
