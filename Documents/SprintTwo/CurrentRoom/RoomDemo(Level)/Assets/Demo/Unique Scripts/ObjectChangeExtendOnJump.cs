...
public class ObjectChangeExtendOnJump : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //a lot of this dim trigger nonsense is just me, TJ, trying to reduce glitches by keeping the platforms flat in case
        //of any glitchy slight variances of height

        if (DimTrig(type)!= flat)
        {
            print("ihatethis");
    ...
    bool DimTrig(string type)
    {
        ...
        if (type == "alternate")
        {
            //if (this.gameObject.transform.localScale.y == 1f / 2f || this.gameObject.transform.localScale.y >= scaleY)
            if (this.gameObject.transform.localScale.y == 1f / 2f)
            {
